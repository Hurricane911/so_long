...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/11/21 17:04:23 by joyim             #+#    #+#             */
/*   Updated: 2024/11/21 17:04:23 by joyim            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	scan_player(t_game *game)
{
	...
	while (game->map.player.y < game->map.rows)
	{
		...
		{
			if (game->map.full[game->map.player.y][game->map.player.x] == 'P')
				break ;
			game->map.player.x++;
		}
		if (game->map.full[game->map.player.y][game->map.player.x] == 'P')
...
void	check_valid_path(t_game *game)
{
	...
	if (game->c_check != 0 || game->e_check != 0)
	{
		ft_error_msg("Error\nPath Invalid", game);
	}
}
